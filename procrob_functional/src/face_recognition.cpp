...
// EDIT_1 -- by alaa -- to send feedback to Client in the case of no detected faces, which will initiate robot motion

      if (goal_id_ == 1)
      {
         feedback_.names.clear();
         feedback_.names.push_back("none");
...
// EDIT_2 -- by alaa -- to notify Client of detecting an unknown face

             if (goal_id_ == 1)
             {
                feedback_.names.clear();
                feedback_.names.push_back("Stranger");
                as_.publishFeedback(feedback_);
             }
